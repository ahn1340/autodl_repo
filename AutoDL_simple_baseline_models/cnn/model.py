...
class ThreeLayerNet(nn.Module):
  def __init__(self):
    ...
    self.batch_size = 32
    self.linear1 = nn.Linear(13 * 32, 200)
    self.linear2 = nn.Linear(200, 200)
    self.linear3 = nn.Linear(200, 30)
    self.relu1 = nn.ReLU()
    self.relu2 = nn.ReLU()
    self.relu3 = nn.ReLU()
...
class Model(algorithm.Algorithm):
  ...
  def __init__(self, metadata):
    ...
    self.pytorchmodel = ThreeLayerNet()
    #self.pytorchmodel = nn.Sequential(
    #  nn.Linear(13 * 32, 200),
    #  nn.ReLU(),
    #  nn.Linear(200, 200),
    #  nn.ReLU(),
  ...
  def get_torch_tensors(self):
    ...
    #mfcc_data = np.swapaxes(features[40], 0, 1)
    #fig, ax = plt.subplots()
    #cax = ax.imshow(mfcc_data, interpolation='nearest', cmap=cm.coolwarm, origin='lower', aspect='auto')
    #ax.set_title('MFCC')
  ...
  def train(self, dataset, remaining_time_budget=None):
    ...
    if steps_to_train <= 0:
      ...
      self.training_data_iterator_uncropped = self.input_function(dataset, is_training=True, return_raw=True)
...
def crop_time_axis(tensor_4d, num_frames, begin_index=None):
  ...
  #TODO: Try to plot the raw wav files to see if padding and cropping works properly.
  # Probably we want to completely rewrite this part
