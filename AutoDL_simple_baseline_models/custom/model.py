...
class torchModel(nn.Module):
  #TODO: Try more complex models that handle sequential data
...
class LSTMmodel(nn.Module):
  def __init__(self, input_dim, output_dim, hidden_dim):
    ...
    self.lstm = nn.LSTM(input_dim, hidden_dim, 5)
  ...
  def forward(self, x):
    x = x.contiguous().view(x.size()[0], x.size()[2], x.size()[1])
    x, _ = self.lstm(x)
    x = self.out(x[:,-1,:])
    x = self.log_softmax(x)
    return x
...
class Model(algorithm.Algorithm):
  ...
  def __init__(self, metadata):
    ...
    # Input dim is now 99 * 13, which is the shape of the mfcc feature.
    ...
    self.hidden_dim = 128 # for LSTM cell
    ...
    self.pytorchmodel = LSTMmodel(self.input_dim, self.output_dim, self.hidden_dim)
  ...
  def get_dataloader(self, tfdataset, batch_size, train=False):
    ...
    if train:
      ...
      ######## Jinu part #############
      # Normalization. Apply independently for each dataset to not assume
      # constant channel condition.
      mean = np.mean(features, axis=2)
      mean = np.repeat(mean[:, :, np.newaxis], self.default_feature_shape[1], axis=2)
      std = np.std(features, axis=2)
      std = np.repeat(std[:, :, np.newaxis], self.default_feature_shape[1], axis=2)
